...
describe('UniversalSerializer', () => {
	...
	describe('replace serializer', () => {
		it('replaces an existing serializer', () => {
			...
			const existingSerializer = sut.replaceSerializer(newFirstSerializer.object);
			...
			firstSerializer.verify(s => s.serialize(trigger.object), Times.never());
			secondSerializer.verify(s => s.serialize(trigger.object), Times.never());
			expect(existingSerializer).to.equal(firstSerializer.object);
		});

		it('throws when no serializer can be replaced', () => {
			const newSerializer = TypeMoq.Mock.ofType<Serializer<Trigger>>();
			newSerializer.setup(s => s.getType()).returns(_ => 'Another');

			expect(() => sut.replaceSerializer(newSerializer.object)).to.throw();
		});
	});
